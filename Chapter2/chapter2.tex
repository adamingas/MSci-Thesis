...
The objects are represented by points/vectors in an $m$-dimensional space ${\bf x}_i \in \mathbb{R}^m$ or by the matrix $X_{ij}$, and their distance by any metric we want. In this case we choose the Euclidean:
\begin{equation}
...
\begin{align}
	S(X_{ij})&= \text{stress of the fixed configuration } {\bf x_1, ...,x_n}\\
...
\begin{equation}
	P(y_i = 1(\text{White})|\bf{x}_i).
\end{equation}  
We can use Bayes' theorem to expand this probability into
\begin{equation}
	P(y_i =1|\bf{x}_i) = \frac{P(\bf{x}_i|y_i)P(y_i)}{P(\bf{x}_i)},
\end{equation}
 where the likelihood is given by $P(x_i|y_i)$, the prior by $P(y_i)$, and the marginal likelihood by $P(y_i)$.
 The marginal likelihood can be obtained by integrating out the class variable
 \begin{equation}
 	P(\bf{x}_i) = \sum_{y_i =0}^{1} P(\bf{x}_i|y_i)P(\bf{x}_i).
 \end{equation} 

To decide in which class a point belongs to, we construct a decision boundary using the discriminant function
\begin{equation}
f(\bf{x}_i)=\log \frac{P(y_i=1 | \bf{x}_i)}{P(y_i = 0 | \bf{x}_i)}.
\label{eq:discr}
\end{equation}
...
A linear function (in parameters) can be used to model \eqref{eq:discr}. The features can be transformed by a non-linear function $\phi(\bf{x}_i) = [\phi_1(\bf{x_i}),\phi_2(\bf{x_i}),...,\phi_D(\bf{x_i})]$ which can span a higher or lower dimensional space. The model is given by
\begin{equation}
	a = \log \frac{P(y_i=1 | \bf{x}_i)}{P(y_i = 0 | \bf{x}_i)} = \phi(\bf{x})\theta
\end{equation}
